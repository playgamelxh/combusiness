...
{
    global $curl, $baseUrl, $db, $agent, $rabbitmqObj, $rabbitmqConfig;

    $list = array();
    while (count($list)<=$curl->maxThread) {
//        $rabbitmqObj = new RabbitMQ($rabbitmqConfig);
        $rs = $rabbitmqObj->get('combusiness_comname_ssdb');
        if (!empty($rs)) {
            $resArr = json_decode($rs, true);
            if(is_numeric($resArr['comname'])) {
                continue;
            }
            $list[] = $resArr;
        } else {
...
//            echo $v['province'], ":", $v['comname'], "\r\n";
            $data = array(
                ...
                'provice' => $v['province'],
                'key'     => $v['comname'],
                'token'   => md5($v['comname'].'2c2c401f52b79fbc9740168d134b8954'),
                'usep'    => '',
            );
            ...
            $task = array(
                'url'  => $baseUrl . "key={$v['comname']}&province={$data['usep']}&token={$data['token']}",
...
//            echo "1:{$args['data']['key']}\r\n";

//            $beand = new Socket_Beanstalk($bean_config);
//            $beand->connect();
//            $beand->choose('get_comname_unique');
...
//            print_r($arr);
//            $beand->put(1024, 0, 1, json_encode($arr));
...
//        echo "2:{$args['data']['key']}\r\n";//return;
        ...
        if(is_numeric($data['key']) && isset($data['id'])){
            $data['key'] = $db->get('company', 'comname', array('id' => $data['id']));
        }else {
            $type = rand(0,1);
            while(true) {
//                if (mb_strlen($data['key'], 'utf-8') > 7) {
//                    $data['key'] = mb_substr($data['key'], 0, 7, 'utf-8');
//
//                } else {
                    $num = 2;
...
//                    return;
        ...
        $token = md5($data['key'] . $data['usep'] . '2c2c401f52b79fbc9740168d134b8954');
        ...
        $task = array(
            'url'  => $baseUrl . "key={$data['key']}&province={$data['usep']}&token={$token}",
            'args' => array('data' => $data),
            'opt'  => array(
                CURLOPT_POST       => 0,
                CURLOPT_HTTPHEADER => $head,
